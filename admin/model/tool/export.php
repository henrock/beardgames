...
class ModelToolExport extends Model {
	...
	protected function getAllLanguageIds( &$database ) {
	...
	function storeProductsIntoDatabase( &$database, &$products ) 
	{
		// find the default language id
		//$languageId = $this->getDefaultLanguageId($database);
		$languageIds = $this->getAllLanguageIds($database);
		...
		foreach ($products as $product) {
			...
			$insert = false;
			if($productId != "") {
				$product_exists = $database->query("SELECT `product_id` FROM `".DB_PREFIX."product` WHERE `product_id` = '$productId'");
				if($product_exists->num_rows > 0) {
					$sql  = "UPDATE `".DB_PREFIX."product` SET `product_id`=$productId,`quantity`=$quantity,`sku`='$sku',`upc`='$upc',`ean`='$ean',`jan`='$jan',`isbn`='$isbn',`mpn`='$mpn',`location`='$location',";
					$sql .= "`stock_status_id`=$stockStatusId,`model`='$model',`manufacturer_id`=$manufacturerId,`image`='$imageName',`shipping`=$shipping,`price`=$price,`points`=$points,`weight`=$weight,`weight_class_id`=$weightClassId,`status`=$status,";
					$sql .= "`tax_class_id`=$taxClassId,`viewed`=$viewed,`length`=$length,`width`=$width,`height`=$height,`length_class_id`='$lengthClassId',`sort_order`='$sort_order',`subtract`='$subtract',`minimum`='$minimum') VALUES ";
					$sql .= "`date_added`=".($dateAdded=='NOW()') ? "$dateAdded," : "'$dateAdded',";
					$sql .= "`date_modified`=".($dateModified=='NOW()') ? "$dateModified," : "'$dateModified',";
					$sql .= "`date_available`=".($dateAvailable=='NOW()') ? "$dateAvailable" : "'$dateAvailable'";
			...
			$insert = false;
			if($productId != "") {
				$product_description_exists = $database->query("SELECT `product_id` FROM `".DB_PREFIX."product_description` WHERE `product_id` = '$productId' AND `language_id` = '$languageId'");
				var_dump($product_description_exists);
				if($product_description_exists->num_rows > 0) {
					$sql2 = "UPDATE `".DB_PREFIX."product_description` SET `product_id`=$productId,`language_id`=$languageId,`name`='$productName',`description`='$productDescription',`meta_description`='$meta_description',`meta_keyword`='$meta_keywords',`tag`='$tags' ";
					$sql2 .= "WHERE `product_id` = '$productId' AND `language_id` = '$languageId'";
					$database->query($sql2);
				} else {
					$insert = true;
				}
	...
	function uploadProducts( &$reader, &$database ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$products[] = $this->moreProductCells( $i, $j, $data, $product );
		}
