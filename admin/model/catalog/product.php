...
class ModelCatalogProduct extends Model {
	...
	public function getProductsMissingWeight() {
	...
	public function getProductsMissingSeoName() {
		$sql = "SELECT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)";
		...
		foreach($query->rows as $product) {
			$sql = "SELECT url_alias_id FROM ".DB_PREFIX."url_alias WHERE query = 'product_id=".$product['product_id']."' AND `keyword` != ''";
			$seo_query = $this->db->query($sql);
			
			if(count($seo_query->rows) == 0) {
				$products[] = $product;
			}
		}
