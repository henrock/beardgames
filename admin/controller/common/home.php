...
class ControllerCommonHome extends Controller {   
	public function index() {
		...
		$results = $this->model_catalog_product->getProductsMissingWeight();
		$this->data['products_missing_weight'] = array();
		foreach($results as $result) {
			$this->data['products_missing_weight'][] = array(
				'name' 		=> $result['name'],
				'edit_link' => $this->url->link('catalog/product/update', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'], 'SSL')
			);
		}

		$results = $this->model_catalog_product->getProductsMissingSeoName();
		$this->data['products_missing_seo_name'] = array();
		foreach($results as $result) {
			$this->data['products_missing_seo_name'][] = array(
				'name' 		=> $result['name'],
				'edit_link' => $this->url->link('catalog/product/update', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'], 'SSL')
