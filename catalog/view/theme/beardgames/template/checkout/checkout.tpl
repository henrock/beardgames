...
$('.checkout-heading a').live('click', function() {
	...
	$(this).parent().parent().find('.checkout-content').show();
});
<?php if (!$logged) { ?> 
	...
	<?php }else{ ?>
		$.ajax({
			...
			success: function(html) {
				...
				$('#payment-address .checkout-content').fadeIn("fast");
			},
			error: function(xhr, ajaxOptions, thrownError) {
				alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
			}
...
$('#button-account').live('click', function() {
	if($('input[name=\'account\']:checked').attr('value') == 'register') {
		document.location = '/account/register?co=1';
	} else {
		$.ajax({
			...
			success: function(html) {
				...
				$('#checkout .checkout-content').fadeOut("fast");
				...
				if (shipping_address) {
					$.ajax({
						...
						success: function(html) {
							...
							$('#payment-address .checkout-content').hide();
