...
class ModelCatalogProduct extends Model {
	...
	public function getLanguages($product_attributes, $skip_swedish = true) {
		$language_array = array();
		foreach($product_attributes as $product_attribute_group) {
			if($product_attribute_group['attribute_group_id'] == 7) {
				foreach($product_attribute_group['attribute'] as $attribute) {
					if($attribute['attribute_id'] == 14) {
						$languages = explode(',', $attribute['text']);
						foreach($languages as $language) {
							$language = trim($language);
							if($skip_swedish) {
								if(!in_array($language, array('svenska','se','Svenska'))) {
	...
	public function getAge($product_attributes) {
		$language_array = array();
		foreach($product_attributes as $product_attribute_group) {
			if($product_attribute_group['attribute_group_id'] == 7) {
				foreach($product_attribute_group['attribute'] as $attribute) {
	...
	public function getContents($product_attributes) {
		$language_array = array();
		foreach($product_attributes as $product_attribute_group) {
			if($product_attribute_group['attribute_group_id'] == 7) {
				foreach($product_attribute_group['attribute'] as $attribute) {
	...
	public function getGameTime($product_attributes) {
		$language_array = array();
		foreach($product_attributes as $product_attribute_group) {
			if($product_attribute_group['attribute_group_id'] == 7) {
				foreach($product_attribute_group['attribute'] as $attribute) {
	...
	public function getPlayers($product_attributes) {
		$language_array = array();
		foreach($product_attributes as $product_attribute_group) {
			if($product_attribute_group['attribute_group_id'] == 7) {
				foreach($product_attribute_group['attribute'] as $attribute) {
