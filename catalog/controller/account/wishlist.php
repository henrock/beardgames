...
class ControllerAccountWishList extends Controller {
	public function index() {
		...
		$this->data['no_image'] = $this->model_tool_image->resize('data/produkter/spel/no-shot.png', $this->config->get('config_image_category_width'), $this->config->get('config_image_category_height'));
		...
		foreach ($this->session->data['wishlist'] as $key => $product_id) {
			...
			if ($product_info) { 
				...
				if ($product_info['quantity'] <= 0) {
					...
					$stock = '<i class="fa fa-cubes fa-2x unavailable" title="Finns ej i lager"></i>';
				} elseif ($this->config->get('config_stock_display')) {
					$stock = $product_info['quantity'];
				} else {
					//$stock = $this->language->get('text_instock');
					$stock = '<i class="fa fa-cubes fa-2x available" title="Finns i lager"></i>';
				}
	...
	public function add() {
		...
		if ($product_info) {
			...
			$json['total_count'] = isset($this->session->data['wishlist']) ? count($this->session->data['wishlist']) : 0;
		}	
