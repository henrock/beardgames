...
class ControllerModuleBestSeller extends Controller {
	protected function index($setting) {
		...
		$this->data['no_image'] = $this->model_tool_image->resize('data/produkter/spel/no-shot.png', $setting['image_width'], $setting['image_height'], "wh");
				
		foreach ($results as $result) {
			if ($result['image']) {
				$image = $this->model_tool_image->resize($result['image'], $setting['image_width'], $setting['image_height'], "wh");
			} else {
				$image = false;
			}
			...
			if(strlen(html_entity_decode($result['name'])) > 40) {
				$name = htmlentities(substr(html_entity_decode($result['name']), 0, 40)."...");
			} else {
				$name = $result['name'];
			}
